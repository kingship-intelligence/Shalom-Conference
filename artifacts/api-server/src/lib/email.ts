...
export async function sendMerchOrderReceived(opts: {
  id: number;
  name: string;
  email: string;
  phone?: string | null;
  ...
  const paymentReference = escapeHtml(opts.paymentReference);
...
<html lang="en"><body style="margin:0;padding:32px 16px;background:#0d0d0d;font-family:Arial,sans-serif;color:#fff;">
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table width="580" style="max-width:580px;width:100%;background:#141414;border-radius:16px;overflow:hidden;">
      <tr><td style="padding:32px;text-align:center;background:linear-gradient(160deg,#1a0a00,#0d0d0d);border-bottom:2px solid #f97316;">
        <p style="margin:0;color:#f97316;font-size:12px;font-weight:700;letter-spacing:4px;text-transform:uppercase;">Shalom Merch</p>
        <h1 style="margin:10px 0 0;font-size:30px;color:#fff;">Preorder Received</h1>
      </td></tr>
      <tr><td style="padding:32px;">
        <p style="font-size:18px;font-weight:700;">Hey ${name},</p>
        <p style="color:#c0c0c0;line-height:1.7;">We received your merch preorder. This email confirms that your order was submitted; finance will verify your Cash App payment and follow up if anything is needed.</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;border-top:1px solid #333;border-bottom:1px solid #333;">${itemRows}
          <tr><td style="padding:14px 0;color:#f97316;font-weight:800;">Total</td><td style="padding:14px 0;color:#f97316;font-weight:800;text-align:right;">$${opts.total.toFixed(2)}</td></tr>
        </table>
        <p style="color:#a0a0a0;font-size:13px;line-height:1.6;">Order #${opts.id}<br />Cash App reference: ${paymentReference}</p>
        <p style="color:#a0a0a0;font-size:13px;line-height:1.6;">Questions? Email <a href="mailto:${FINANCE_EMAIL}" style="color:#f97316;">${FINANCE_EMAIL}</a>.</p>
      </td></tr>
    </table>
  </td></tr></table>
</body></html>`;
